...
int findEnd (int root, vector<vector<int>> tree, int n, bool visited[]){
    int answer = -1;
    visited[root] = true;
    ...
    while (!q.empty()){
        ...
        // answer.push_back(cNode);
        if (temp.find(cNode)==temp.end()) temp[cNode] = cHeight;
       // std::cout<<cNode<<" is the node and distance from root = "<<cHeight<<std::endl;
        for (int j=0;j<n;j++){
    ...
    for (auto it = temp.begin();it!=temp.end();it++){
        if (it->second>maxim) maxim = it->second;
    }
    for (auto it = temp.begin();it!=temp.end();it++){
        if (it->second==maxim) {
            return it->first;
...
vector<int> findPath(int source, int destination, vector<vector<int>> tree, int n, bool visited[]){
    vector<int> answer;
    ...
    visited[source] = true;
    for (int j=0;j<n;j++){
        if (!tree[source][j])continue;
        vector<int> remPath = findPath(j,destination,tree,n,visited);
        if (remPath.size()==0) continue;
        remPath.insert(remPath.begin(),source);
        return remPath;
    }
    return answer;
}
...
vector<int> findMinHeightTrees(int n, vector<vector<int>>& edges){
    ...
    bool visited[n];
    for (int i=0;i<n;i++) visited[i] = false;
    int end = findEnd(root,tree,n,visited);
    std::cout<<end<<std::endl;
    reset(visited,n);
    vector<int> nodes = findPath(root,end,tree,n,visited);
    // std::cout<<"did we get here 3?"<<std::endl;
    // for (auto val : nodes) std::cout<<val<<" ";
    // std::cout<<std::endl;
    return nodes;
}
